...
import { Menu, Globe, Store, PlusCircle, Cloud, RefreshCw, CheckCircle2, Crown, UserCheck, LogOut, Lock, Download, Search, Database, CreditCard, Zap, Sun, Moon, Coins, Bell, ShoppingCart, Barcode, Eye, MessageCircle, BookOpen } from 'lucide-react';
...
import { CardPassbookView } from './components/CardPassbookView';
import { FinanceCalculatorView } from './components/FinanceCalculatorView';
...
import { RealtimeOrderBanner } from './components/RealtimeOrderBanner';
import { FurnitureJobCardTracker } from './components/FurnitureJobCardTracker';
import { FinanceDORegisterView } from './components/FinanceDORegisterView';
import { DailyCashReconciliationModal } from './components/DailyCashReconciliationModal';
import { BarcodeScannerModal } from './components/BarcodeScannerModal';
import { OwnerDailyDigestModal } from './components/OwnerDailyDigestModal';
import { DailyCollectionRegisterView } from './components/DailyCollectionRegisterView';
import { VillageKhataRouteView } from './components/VillageKhataRouteView';
import { LuckyDrawModal } from './components/LuckyDrawModal';
import { SchemeMaturityModal } from './components/SchemeMaturityModal';
import { WarrantyTrackerModal } from './components/WarrantyTrackerModal';
import { SCHEMES_CONFIG, GLOBAL_ZERO_RESET_KEY } from './utils/storage';
...
import { getStoredOrderNotifications } from './utils/notifications';
import { DuePaymentRemindersModal } from './components/DuePaymentRemindersModal';
import { MobilePocketKhataView } from './components/MobilePocketKhataView';
import { WhatsAppCardGroupInviteModal } from './components/WhatsAppCardGroupInviteModal';
import { WeddingPackageQuotationModal } from './components/WeddingPackageQuotationModal';
import { FestivalAndWishesModal } from './components/FestivalAndWishesModal';
...
export default function App() {
  ...
  // Visitors without an active session will see the direct login screen
  ...
  const [showDailyCashModal, setShowDailyCashModal] = useState<boolean>(false);
  const [showBarcodeModal, setShowBarcodeModal] = useState<boolean>(false);
  const [showOwnerDigestModal, setShowOwnerDigestModal] = useState<boolean>(false);
  const [showLuckyDrawModal, setShowLuckyDrawModal] = useState<boolean>(false);
  const [showSchemeMaturityModal, setShowSchemeMaturityModal] = useState<boolean>(false);
  ...
  const [financeCalcPreload, setFinanceCalcPreload] = useState<{
    productName?: string;
    productPrice?: number;
    customerName?: string;
  ...
  const handleOpenFinanceWithDetails = (details?: {
    productName?: string;
    productPrice?: number;
    customerName?: string;
    customerPhone?: string;
    ...
    if (details) {
      setFinanceCalcPreload(details);
    }
    setActiveTab('finance-calc');
  };

  const [editingTransaction, setEditingTransaction] = useState<TransactionEntry | null>(null);
  const [confirmingOrder, setConfirmingOrder] = useState<TransactionEntry | null>(null);
  const [showNotificationsDropdown, setShowNotificationsDropdown] = useState<boolean>(false);
  const [showDueRemindersModal, setShowDueRemindersModal] = useState<boolean>(false);
  const [showMobilePocketKhata, setShowMobilePocketKhata] = useState<boolean>(false);
  const [showWhatsAppGroupModal, setShowWhatsAppGroupModal] = useState<boolean>(false);
  const [showWeddingQuotationModal, setShowWeddingQuotationModal] = useState<boolean>(false);
  const [showFestivalWishesModal, setShowFestivalWishesModal] = useState<boolean>(false);

  const pendingCartOrdersCount = useMemo(() => {
    return (db.transactions || []).filter(
      (t) =>
        (t.source === 'online_cart' || t.orderStatus === 'pending' || t.invoiceNo?.startsWith('INV-ORD')) &&
        (!t.serialNumber || t.orderStatus === 'pending')
    ).length;
  }, [db.transactions]);

  const handleOpenConfirmOrder = (target: TransactionEntry | string) => {
    if (typeof target === 'string') {
      const found = (db.transactions || []).find((t) => t.invoiceNo === target || t.id === target);
      if (found) {
        setConfirmingOrder(found);
      } else {
        const stored = getStoredOrderNotifications();
        const n = stored.find((item) => item.invoiceNo === target || item.id === target);
        if (n) {
          setConfirmingOrder({
            id: n.id,
            ...
            village: n.customerAddress,
            totalAmount: n.totalAmount,
            payingNow: n.downPayment || 0,
            dueAmount: Math.max(0, n.totalAmount - (n.downPayment || 0)),
            paymentMode: (n.paymentMode as any) || 'Online',
            itemDetails: n.itemSummary,
            itemsDetail: n.itemsDetail,
            deliveryAddress: n.customerAddress,
            orderStatus: (n.orderStatus as any) || 'pending',
            source: 'online_cart',
      ...
      setConfirmingOrder(target);
    }
  };

  const handleSaveConfirmedOrder = (updatedOrder: TransactionEntry) => {
    handleUpdateTransaction(updatedOrder);
  ...
  useEffect(() => {
    ...
    if (tabParam) {
      if (
        ...
        tabParam === 'card-passbook' ||
        tabParam === 'finance-calc' ||
    ...
    const passbookParam = params.get('passbook');
    if (passbookParam) {
      setActiveTab('card-passbook');
      setAppMode('erp');
    } else if (actionParam === 'collection') {
  ...
  const safeMergeCollections = <T extends Record<string, any>>(
    ...
    if (preferRemote) {
      // 1. Put local entries first
      localList.forEach((item) => {
        const key = getKey(item);
        if (key) map.set(key, item);
      });
      // 2. Remote entries update/add on top of local (ensures mobile updates reflect in real time without reverting)
      ...
      // 2. Local entries take precedence (e.g. startup IndexedDB load or local CSV import)
      localList.forEach((item) => {
        const key = getKey(item);
        if (key) {
          const existing = map.get(key);
      ...
      (remoteData) => {
        if (remoteData && !isResettingRef.current) {
          ...
          setDb((prev) => {
            ...
            const preferRemote = !isRecentLocalImport;

            const mergedTransactions = isRecentLocalImport && (prev.transactions || []).length > 0
              ? safeMergeCollections(prev.transactions || [], remoteData.transactions || [], (t) => (t.invoiceNo ? `inv_${t.invoiceNo.toLowerCase().trim()}` : t.id || ''), false)
    ...
    setDb((prev) => {
      ...
      const updatedCustomers = prev.customers.map((c) => {
        if (
          ...
          return {
            ...
            village: entryData.village || c.village,
            linkedCardNumber: entryData.cardNumber || c.linkedCardNumber,
            linkedSchemeId: entryData.schemeId || c.linkedSchemeId,
      ...
      if (!customerExists) {
        const newCustomer: Customer = {
          ...
          linkedCardNumber: entryData.cardNumber,
          linkedSchemeId: entryData.schemeId,
          lastVisit: entryData.date,
        };
      ...
      const deductFromItem = (s: StockItem, qty: number): StockItem => {
        let currentShop = s.shopQty !== undefined ? s.shopQty : s.quantity;
        let currentGodown = s.godownQty !== undefined ? s.godownQty : 0;
        if (currentShop >= qty) {
          currentShop -= qty;
        } else {
          const remaining = qty - currentShop;
          currentShop = 0;
          currentGodown = Math.max(0, currentGodown - remaining);
        }
        return {
          ...s,
          shopQty: currentShop,
          godownQty: currentGodown,
          quantity: currentShop + currentGodown,
        };
      };
      ...
      if (entryData.itemsDetail && entryData.itemsDetail.length > 0) {
        ...
        if (deductMap.size > 0) {
          updatedStock = prev.stock.map((s) => {
            if (deductMap.has(s.id)) {
              return deductFromItem(s, deductMap.get(s.id) || 1);
            }
            return s;
          });
        }
        ...
        updatedStock = prev.stock.map((item) => {
          if (item.id === entryData.stockItemId) {
            return deductFromItem(item, entryData.quantity || 1);
          }
          return item;
        });
      }

      // 3. Link sale / goods taken to Card Member passbook if cardholder is matched
      let updatedCardMembers = prev.cardMembers || [];
      let updatedCardTransactions = prev.cardTransactions || [];

      const normCustomerName = (entryData.customerName || '').trim().toLowerCase();
      const normCustomerPhone = (entryData.customerPhone || '').trim().replace(/\D/g, '');
      const explicitCardNo = entryData.cardNumber ? Number(entryData.cardNumber) : undefined;

      const matchingMember = (prev.cardMembers || []).find((m) => {
        if (explicitCardNo && Number(m.cardNumber) === explicitCardNo) return true;
        if (m.customerName && m.customerName.trim().toLowerCase() === normCustomerName) return true;
        if (normCustomerPhone && normCustomerPhone.length >= 8 && m.phone) {
          const mPhone = m.phone.trim().replace(/\D/g, '');
          if (mPhone.includes(normCustomerPhone) || normCustomerPhone.includes(mPhone)) return true;
        }
        return false;
      });

      if (matchingMember) {
        const goodsAmount = entryData.totalAmount || 0;
        const itemDesc =
          entryData.itemDetails ||
          (entryData.itemsDetail && entryData.itemsDetail.length > 0
            ? entryData.itemsDetail.map((i) => `${i.productName || 'वस्तू'} x${i.quantity || 1}`).join(', ')
            : 'गृहोपयोगी वस्तू / साहित्य');

        const newCardTx: CardTransaction = {
          id: `card-tx-sale-${newEntry.id}`,
          cardId: matchingMember.id,
          cardNumber: matchingMember.cardNumber,
          schemeId: matchingMember.schemeId,
          customerName: matchingMember.customerName,
          customerPhone: matchingMember.phone || entryData.customerPhone,
          receiptNo: entryData.invoiceNo,
          date: entryData.date,
          type: 'GoodsTaken',
          amount: goodsAmount,
          paymentMode: (entryData.paymentMode === 'Online' ? 'Online' : 'Cash'),
          agentName: matchingMember.agentName || 'Store Billing',
          remarks: `वस्तू उचल / विक्री बिल: ${itemDesc} (बिल #${entryData.invoiceNo})`,
          goodsDetail: itemDesc,
          balanceAfter: (matchingMember.netBalance || 0) - goodsAmount,
          createdAt: newEntry.createdAt,
        };

        updatedCardTransactions = [newCardTx, ...updatedCardTransactions];

        updatedCardMembers = (prev.cardMembers || []).map((m) => {
          if (m.id === matchingMember.id) {
            const newGoods = (m.totalGoodsTaken || 0) + goodsAmount;
            return {
              ...m,
              totalGoodsTaken: newGoods,
              netBalance: (m.totalDeposited || 0) - ((m.totalRefunded || 0) + newGoods),
            };
          }
          return m;
        });
      ...
      return {
        ...
        cardTransactions: updatedCardTransactions,
      };
    });
  };
  ...
  const handleUpdateStockLocations = (id: string, shopQty: number, godownQty: number) => {
    setDb((prev) => ({
      ...prev,
  ...
  const handleTransferStock = (transferData: Omit<StockTransferRecord, 'id' | 'timestamp'>) => {
  ...
  // Furniture Job Cards Handlers
  const handleSaveFurnitureJobCard = (card: FurnitureJobCard) => {
    setDb((prev) => {
      const existing = (prev.furnitureJobCards || []).some((c) => c.id === card.id);
      const updatedList = existing
        ? (prev.furnitureJobCards || []).map((c) => (c.id === card.id ? card : c))
        : [card, ...(prev.furnitureJobCards || [])];
      return {
        ...prev,
        furnitureJobCards: updatedList,
      };
    });
  };

  const handleUpdateFurnitureStage = (id: string, newStage: FurnitureJobCard['stage']) => {
    setDb((prev) => ({
      ...prev,
      furnitureJobCards: (prev.furnitureJobCards || []).map((c) =>
        c.id === id ? { ...c, stage: newStage, updatedAt: new Date().toISOString() } : c
      ),
    }));
  };

  // Finance DO Register Handlers
  const handleSaveFinanceDORecord = (record: FinanceDORecord) => {
    setDb((prev) => {
      const existing = (prev.financeDORecords || []).some((r) => r.id === record.id);
      const updatedList = existing
        ? (prev.financeDORecords || []).map((r) => (r.id === record.id ? record : r))
        : [record, ...(prev.financeDORecords || [])];
      return {
        ...prev,
        financeDORecords: updatedList,
      };
    });
  };

  const handleUpdateFinanceDOStatus = (id: string, status: FinanceDORecord['payoutStatus'], utr?: string) => {
    setDb((prev) => ({
      ...prev,
      financeDORecords: (prev.financeDORecords || []).map((r) =>
        r.id === id
          ? {
              ...r,
              payoutStatus: status,
              ...(utr ? { utrNumber: utr } : {}),
              ...(status === 'Disbursed to Bank' || status === 'UTR Received'
  ...
  // Cash Reconciliation Handler
  const handleSaveCashReconciliation = (record: DailyCashReconciliation) => {
    setDb((prev) => ({
      ...prev,
      cashReconciliations: [record, ...(prev.cashReconciliations || [])],
    }));
  };
  ...
  const handleRecordCardTransaction = (txData: Omit<CardTransaction, 'id' | 'createdAt'>) => {
    setDb((prev) => {
      // 1. Anti-Double-Submission Guard:
      // If a transaction for this exact member/card with same amount, type, and date was created within last 8 seconds, ignore duplicate tap!
      const isDuplicate = (prev.cardTransactions || []).some((existingTx) => {
        if (
          existingTx.cardNumber === txData.cardNumber &&
          existingTx.schemeId === txData.schemeId &&
          existingTx.type === txData.type &&
          existingTx.amount === txData.amount &&
          existingTx.date === txData.date
        ) {
          const timeDiff = Math.abs(Date.now() - new Date(existingTx.createdAt).getTime());
          if (timeDiff < 8000) {
            return true;
          }
        }
      ...
      if (isDuplicate) {
        console.warn('Duplicate card transaction tap prevented:', txData);
        return prev;
      }

      // 2. Sanitize existing card transactions: any accidental "1080" or "1079" against-bill number on card weekly payment is converted to CR-0001
      const cleanedExistingTxs = (prev.cardTransactions || []).map((t) => {
        if (t.receiptNo === '1080' || t.receiptNo === '1079') {
          return { ...t, receiptNo: 'CR-0001' };
        }
        return t;
      });

      // 3. Ensure this card transaction has a proper CR-xxxx receipt number and never 1080
      let safeReceiptNo = (txData.receiptNo || '').trim();
      if (!safeReceiptNo || safeReceiptNo === '1080' || safeReceiptNo === '1079') {
        safeReceiptNo = getNextReceiptNumber(cleanedExistingTxs);
      }
      ...
      return {
        ...prev,
        cardTransactions: [newTx, ...cleanedExistingTxs],
        cardMembers: updatedMembers,
      };
    });
  };
  ...
  const handleRequestStaffApproval = (newStaff: { name: string; phone: string; role: string }) => {
    const pendingStaff: StaffMember = {
      id: `stf-req-${Date.now()}`,
      name: newStaff.name,
  ...
  if (appMode === 'shop') {
    return (
      <>
        <RealtimeOrderBanner
          onConfirmOrder={(invNo) => {
            setAppMode('erp');
            handleOpenConfirmOrder(invNo);
          }}
          onViewInvoice={(invNo) => {
            const found = (db.transactions || []).find((t) => t.invoiceNo === invNo);
        ...
        <ShopLandingView
          ...
          salesTransactions={db.transactions}
        ...
        {showLoginModal && (
          <LoginModal
            ...
            onViewCustomerShop={() => setShowLoginModal(false)}
            onLoginSuccess={(user) => {
              setCurrentUser(user);
            ...
            adminName={db.settings.ownerName || 'Shubham Shende (Admin)'}
            adminPassword={db.settings.adminPassword || 'Saksham@291022'}
            staffPassword={db.settings.staffPassword || 'staff'}
            staffList={db.staff}
            onRequestStaffApproval={handleRequestStaffApproval}
          />
        )}
      </>
    );
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-[#0B1528] flex items-center justify-center p-2 sm:p-4">
        <LoginModal
          isOpen={true}
          isFullScreen={true}
          canClose={true}
  ...
  return (
    <div className="min-h-screen bg-[#F5EFEB] dark:bg-[#0B1120] flex text-slate-800 dark:text-slate-100 antialiased font-sans transition-colors duration-300">
      <RealtimeOrderBanner
        onConfirmOrder={handleOpenConfirmOrder}
        onViewInvoice={(invNo) => {
          const found = (db.transactions || []).find((t) => t.invoiceNo === invNo);
          if (found) setSelectedInvoice(found);
        }}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto pb-20 lg:pb-0">
        {/* Top Navbar */}
        <header className="sticky top-0 z-30 bg-white/95 dark:bg-[#0F172A]/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-3 sm:px-6 py-2 sm:py-3 flex items-center justify-between no-print">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <button
              onClick={() => setIsMobileOpen(true)}
              className="p-1.5 sm:p-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 lg:hidden cursor-pointer shrink-0"
            ...
            <div className="flex items-center gap-2 min-w-0">
              <span className="font-bold text-slate-900 dark:text-white text-sm sm:text-base tracking-tight truncate max-w-[130px] sm:max-w-xs md:max-w-none">
                {db.settings.businessName}
              </span>
              <span className="hidden sm:inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-mono font-medium border border-slate-200 dark:border-slate-700 shrink-0">
                Cloud ERP
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            {/* Online Cart Orders Notification Button with Counter Badge */}
            <button
              type="button"
              onClick={() => setActiveTab('online-orders')}
              title="नवीन ऑनलाईन ऑर्डर्स तपासा व मॉडेल/सिरीयल नंबर भरा"
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-xs font-bold transition cursor-pointer shadow-2xs ${
                activeTab === 'online-orders'
                  ...
                  : 'border-emerald-300 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/60 hover:bg-emerald-100 text-emerald-900 dark:text-emerald-200'
              }`}
            >
              <ShoppingCart className="w-3.5 h-3.5" />
              <span className="hidden md:inline">ऑर्डर्स</span>
              {pendingCartOrdersCount > 0 && (
                <span className="min-w-[18px] h-[18px] px-1 bg-rose-500 text-white rounded-full text-[10px] font-black flex items-center justify-center animate-pulse">
                  {pendingCartOrdersCount}
                </span>
              )}
            </button>

            {/* WhatsApp Due & Installment Reminders Hub */}
            <button
              type="button"
              onClick={() => setShowDueRemindersModal(true)}
              title="थकीत हप्ता व उधारी WhatsApp रिमाइंडर्स (Due Reminders Hub)"
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-teal-300 dark:border-teal-800 bg-teal-50 dark:bg-teal-950/60 hover:bg-teal-100 text-teal-900 dark:text-teal-200 text-xs font-bold transition cursor-pointer shadow-2xs"
            >
              <MessageCircle className="w-3.5 h-3.5 text-teal-600 dark:text-teal-400" />
              <span className="hidden md:inline">बाकी रिमाइंडर्स</span>
            </button>

            {/* Mobile Pocket Khata & Card Dues Hub */}
            <button
              type="button"
              onClick={() => setShowMobilePocketKhata(true)}
              title="मोबाईल पॉकेट डायरी (लहान स्क्रीनवर १-टॅप हप्ता व उधारी हिशोब)"
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-indigo-300 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-950/60 hover:bg-indigo-100 text-indigo-900 dark:text-indigo-200 text-xs font-bold transition cursor-pointer shadow-2xs"
            >
              <BookOpen className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
              <span>पॉकेट डायरी</span>
            </button>

            {/* WhatsApp Card Group Direct Connect & Bulk VCF Export */}
            <button
              type="button"
              onClick={() => setShowWhatsAppGroupModal(true)}
              title="कार्डधारक अधिकृत WhatsApp ग्रुप जोडा व सर्व Contacts (.vcf) डाऊनलोड करा"
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-emerald-300 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/60 hover:bg-emerald-100 text-emerald-900 dark:text-emerald-200 text-xs font-bold transition cursor-pointer shadow-2xs"
            >
              <MessageCircle className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
              <span className="hidden lg:inline">WhatsApp ग्रुप</span>
            </button>

            {/* Install Mobile App button (Desktop only, mobile has it in bottom nav/sidebar) */}
            <button
              type="button"
            ...
            {/* View Customer Website button: Compact icon on mobile, full on desktop */}
            <button
              type="button"
              onClick={() => setAppMode('shop')}
              title="View Customer Website & Public Passbook Portal"
              className="flex items-center gap-1 px-2 sm:px-2.5 py-1.5 rounded-lg border border-amber-300 bg-amber-50 hover:bg-amber-100 text-amber-900 text-xs font-bold transition cursor-pointer shadow-2xs"
            >
            ...
            {/* Real-time Google Cloud Sync Status Badge (Desktop only) */}
            <button
              type="button"
            ...
            >
              ...
              <span>
                ...
                {cloudStatus === 'offline' && (isFirestoreQuotaExhausted() ? 'स्थानिक सुरक्षित (Quota Reached)' : 'Offline')}
            ...
            {currentUser ? (
              <div className="flex items-center gap-1 sm:gap-1.5 pl-0.5 sm:pl-1">
                <button
                  ...
                  className={`flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1.5 rounded-lg border text-xs font-semibold transition cursor-pointer ${
                ...
                >
                  ...
                  <span className="text-[11px] sm:text-xs">{currentUser.role === 'admin' ? 'Admin' : 'Staff'}</span>
              ...
              <button
                type="button"
                onClick={() => setShowLoginModal(true)}
                className="flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold shadow-xs transition cursor-pointer"
              >
        ...
        <QuickActionsBar
          ...
          onOpenDailyCollectionLog={() => setActiveTab('daily-collection-log')}
          onOpenVillageKhata={() => setActiveTab('village-khata')}
          onOpenCardPassbook={() => setActiveTab('card-passbook')}
          ...
          onOpenBarcodeScanner={() => setShowBarcodeModal(true)}
          onOpenCashReconciliation={() => setShowDailyCashModal(true)}
          onOpenOwnerDigest={() => setShowOwnerDigestModal(true)}
          onOpenFurnitureJobs={() => setActiveTab('furniture-jobs')}
          onOpenFinanceDO={() => setActiveTab('finance-do')}
          onOpenMasterSearch={() => setActiveTab('uploaded-data')}
          onOpenAgentCommission={() => setActiveTab('agent-commission')}
          onOpenFinanceCalc={() => handleOpenFinanceWithDetails({ initialTab: 'schedule' })}
          onOpenLuckyDraw={() => setShowLuckyDrawModal(true)}
          onOpenSchemeMaturity={() => setShowSchemeMaturityModal(true)}
          onOpenWarrantyTracker={() => setShowWarrantyModal(true)}
          onOpenWeddingQuotation={() => setShowWeddingQuotationModal(true)}
          onOpenFestivalWishes={() => setShowFestivalWishesModal(true)}
          onOpenWhatsAppGroup={() => setShowWhatsAppGroupModal(true)}
        />
        ...
        <main className="flex-1 pb-24 md:pb-12">
          ...
          {activeTab === 'add-entry' && (
            <AddEntryView
              ...
              onOpenInvoiceModal={setSelectedInvoice}
              onNavigateFinanceCalc={handleOpenFinanceWithDetails}
            />
          )}
          ...
          {activeTab === 'card-scheme' && (
            <CardSchemeView
              ...
              onNavigateFinanceCalc={() => handleOpenFinanceWithDetails({ initialTab: 'schedule' })}
              salesBills={db.transactions}
            />
          )}

          {activeTab === 'card-passbook' && (
            <CardPassbookView
              cardMembers={db.cardMembers || []}
              transactions={db.cardTransactions || []}
              salesTransactions={db.transactions || []}
              settings={db.settings}
              activeAgent={currentUser?.name}
              onRecordTransaction={handleRecordCardTransaction}
              onUpdateMember={handleUpdateCardMember}
              onOpenNewCardModal={() => {
                setFieldQuickActionTab('new-card');
              ...
              onNavigateSchemeTable={() => setActiveTab('card-scheme')}
              onNavigateFinanceCalc={() => handleOpenFinanceWithDetails({ initialTab: 'schedule' })}
            />
          )}
          ...
          {activeTab === 'finance-calc' && (
            <FinanceCalculatorView
              ...
              initialProductName={financeCalcPreload?.productName || 'Smart LED TV 43" 4K'}
              initialProductPrice={financeCalcPreload?.productPrice || 25000}
              initialCustomerName={financeCalcPreload?.customerName || ''}
              initialCustomerPhone={financeCalcPreload?.customerPhone || ''}
              initialPartnerId={financeCalcPreload?.partnerId || 'bajaj'}
              initialTab={financeCalcPreload?.initialTab || 'schedule'}
            />
          )}
          ...
          {activeTab === 'online-orders' && (
            <OnlineOrdersView
              ...
              onEditTransaction={(entry) => setEditingTransaction(entry)}
              settings={db.settings}
              stock={db.stock}
            />
          )}
          ...
          {activeTab === 'all-entries' && (
            <AllEntriesView
              ...
              onOpenInvoiceModal={setSelectedInvoice}
              onEditEntry={(entry) => setEditingTransaction(entry)}
              onConfirmOrder={handleOpenConfirmOrder}
              settings={db.settings}
            />
          )}
          ...
          {(activeTab === 'furniture-job-cards' || activeTab === 'furniture-jobs') && (
            <FurnitureJobCardTracker
              jobCards={db.furnitureJobCards || []}
              onSaveJobCard={handleSaveFurnitureJobCard}
              onUpdateStage={handleUpdateFurnitureStage}
              businessPhone={db.settings.phone || '8766486915'}
            />
          )}

          {(activeTab === 'finance-do-register' || activeTab === 'finance-do') && (
            <FinanceDORegisterView
              records={db.financeDORecords || []}
              onSaveRecord={handleSaveFinanceDORecord}
              onUpdateStatus={handleUpdateFinanceDOStatus}
              businessPhone={db.settings.phone || '8766486915'}
            />
          )}
          ...
          {activeTab === 'daily-reconciliation' && (
            <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-4">
              <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 shadow-sm text-center space-y-4">
                ...
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">दैनिक गल्ला व कॅश मेळ (Daily Cash Reconciliation)</h2>
                <p className="text-sm text-slate-600 dark:text-slate-400 max-w-md mx-auto">
                  दिवसाच्या अखेरीस प्रत्यक्ष नोटांची मोजणी (₹500, ₹200, ₹100, इ.), ऑनलाईन व रोकड जमा-खर्चाचा ताळमेळ घाला.
                </p>
                <div className="pt-2 flex flex-wrap items-center justify-center gap-3">
                  <button
                    onClick={() => setShowDailyCashModal(true)}
                    className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm shadow-md transition cursor-pointer flex items-center gap-2"
                  >
                    <Coins className="w-4 h-4" />
                    <span>ओपन कॅश टॅली व नोटा मोजणी शीट</span>
                  </button>
                  <button
                    onClick={() => setShowOwnerDigestModal(true)}
                    className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm shadow-md transition cursor-pointer flex items-center gap-2"
                  >
                    <span>दैनिक मालक रिपोर्ट (Daily Digest)</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'daily-collection-log' && (
            <DailyCollectionRegisterView
              ...
              staff={db.staff || []}
              settings={db.settings}
              onOpenCardMember={() => setActiveTab('card-passbook')}
              onOpenCustomerLedger={() => setActiveTab('customers')}
            />
          )}

          {activeTab === 'village-khata' && (
            <VillageKhataRouteView
              customers={db.customers || []}
              cardMembers={db.cardMembers || []}
              transactions={db.transactions || []}
              billReceipts={db.billReceipts || []}
              ...
              staff={db.staff || []}
              settings={db.settings}
              onOpenCardMemberPassbook={() => setActiveTab('card-passbook')}
              onOpenCustomerLedger={() => setActiveTab('customers')}
      ...
      {/* Confirm Online Cart Order Modal (assigning model number & serial number) */}
      {confirmingOrder && (
        <ConfirmOrderModal
          isOpen={Boolean(confirmingOrder)}
          onClose={() => setConfirmingOrder(null)}
          order={confirmingOrder}
          onSaveOrder={handleSaveConfirmedOrder}
      ...
      <MobileBottomNav
        ...
        onOpenPocketKhata={() => setShowMobilePocketKhata(true)}
        cloudStatus={cloudStatus}
      ...
      {/* Daily Cash Reconciliation & Tally Modal */}
      {showDailyCashModal && (
        <DailyCashReconciliationModal
          isOpen={showDailyCashModal}
          onClose={() => setShowDailyCashModal(false)}
          transactions={db.transactions || []}
          cardTransactions={db.cardTransactions || []}
          billReceipts={db.billReceipts || []}
          expenses={db.expenses || []}
          onSaveReconciliation={handleSaveCashReconciliation}
          businessName={db.settings.businessName || 'Shri Sai Enterprises'}
          businessPhone={db.settings.phone || '8766486915'}
        />
      )}

      {/* Barcode & Serial Number Scanner Modal */}
      {showBarcodeModal && (
        <BarcodeScannerModal
          ...
          stock={db.stock || []}
          transactions={db.transactions || []}
          onSelectSerialNumber={(serial, item) => {
      ...
      {/* Owner Daily WhatsApp / Print Digest Modal */}
      {showOwnerDigestModal && (
        <OwnerDailyDigestModal
          isOpen={showOwnerDigestModal}
          onClose={() => setShowOwnerDigestModal(false)}
          transactions={db.transactions || []}
          cardTransactions={db.cardTransactions || []}
          billReceipts={db.billReceipts || []}
          expenses={db.expenses || []}
      ...
      {/* Lucky Draw & Gift Delivery Modal */}
      {showLuckyDrawModal && (
        <LuckyDrawModal
          isOpen={showLuckyDrawModal}
          onClose={() => setShowLuckyDrawModal(false)}
          cardMembers={db.cardMembers || []}
          schemesConfig={SCHEMES_CONFIG}
          businessName={db.settings.businessName || 'Shri Sai Enterprises'}
          businessPhone={db.settings.phone || '8766486915'}
        />
      )}

      {/* 30-Month Scheme Maturity & Gold/Gift Settlement Modal */}
      {showSchemeMaturityModal && (
        <SchemeMaturityModal
          isOpen={showSchemeMaturityModal}
          onClose={() => setShowSchemeMaturityModal(false)}
          cardMembers={db.cardMembers || []}
          settings={db.settings}
          stockItems={db.stock || []}
          onCompleteMaturity={(data) => {
            handleUpdateCardMember(data.memberId, {
              status: 'Completed',
              notes: `${data.productDelivered} सुपूर्द (Maturity Settle) - ₹${data.productValue}`
            });
            setShowSchemeMaturityModal(false);
          }}
        />
      )}

      {/* Appliance & Electronics Warranty Tracker & Claims Modal */}
      {showWarrantyModal && (
      ...
      {/* WhatsApp Due & Installment Payment Reminders Hub */}
      {showDueRemindersModal && (
        <DuePaymentRemindersModal
          isOpen={showDueRemindersModal}
          onClose={() => setShowDueRemindersModal(false)}
          customers={db.customers || []}
          cardMembers={db.cardMembers || []}
          settings={db.settings}
        />
      ...
      {/* Mobile Pocket Khata & Card Dues View (Small Screen Optimized) */}
      {showMobilePocketKhata && (
        <MobilePocketKhataView
          isOpen={showMobilePocketKhata}
          onClose={() => setShowMobilePocketKhata(false)}
          cardMembers={db.cardMembers || []}
          cardTransactions={db.cardTransactions || []}
          customers={db.customers || []}
          salesTransactions={db.transactions || []}
      ...
      {/* WhatsApp Card Group Invite & Bulk Contacts Modal */}
      ...
      {/* Daily Cash Reconciliation & Physical Note Tally Modal */}
      {showDailyCashModal && (
        <DailyCashReconciliationModal
          isOpen={showDailyCashModal}
          onClose={() => setShowDailyCashModal(false)}
          ...
          onSaveReconciliation={() => {
            setShowDailyCashModal(false);
          }}
        />
      )}

      {/* Wedding Furniture & Appliances Package Quotation Generator */}
      {showWeddingQuotationModal && (
        <WeddingPackageQuotationModal
          isOpen={showWeddingQuotationModal}
          onClose={() => setShowWeddingQuotationModal(false)}
          settings={db.settings}
          stockItems={db.stock || []}
        />
      )}

      {/* Festival Greetings & Special Customer Offers Hub */}
      {showFestivalWishesModal && (
        <FestivalAndWishesModal
          isOpen={showFestivalWishesModal}
          onClose={() => setShowFestivalWishesModal(false)}
          cardMembers={db.cardMembers || []}
          customers={db.customers || []}
          settings={db.settings}
        />
