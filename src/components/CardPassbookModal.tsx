...
export const CardPassbookModal: React.FC<CardPassbookModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl max-w-4xl w-full p-4 sm:p-6 space-y-5 shadow-2xl my-auto max-h-[94vh] flex flex-col">
        
        {/* Header */}
        <div className="flex items-start justify-between border-b border-slate-200 pb-3">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="px-3 py-1 rounded-lg bg-blue-700 text-white font-mono font-bold text-sm shadow-xs">
            ...
            <div className="text-xs text-slate-500 mt-1 font-medium flex flex-wrap items-center gap-2">
              {member.phone && <span className="font-mono text-slate-700 font-semibold">📞 {member.phone}</span>}
              {member.village && (
              ...
              {member.sheetNo && (
                <span className="bg-purple-50 text-purple-700 px-2 py-0.5 rounded font-semibold text-[11px] border border-purple-200 font-mono inline-flex items-center gap-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="p-1 rounded-lg text-slate-400 hover:text-slate-800 font-bold cursor-pointer hover:bg-slate-100"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="bg-gradient-to-r from-emerald-50 via-teal-50 to-blue-50 border border-emerald-200 p-3 rounded-xl shadow-xs">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-lg bg-slate-900 hover:bg-black text-white text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-xs transition-all active:scale-95 border border-slate-700"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          ...
          <div className="p-3 bg-blue-50/80 border border-blue-200 rounded-xl">
            <span className="text-blue-700 font-medium block text-[11px] flex items-center gap-1">
          ...
          <div className={`p-3 rounded-xl border ${
            ...
            <span className={`text-lg font-black mt-0.5 block ${netOutstandingBalance > 0 ? 'text-rose-700' : 'text-emerald-700'}`}>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                    ...
                    <td className="p-2.5 text-slate-700">
                      {tx.remarks ||
                        (tx.type === 'Fee'
                          ...
                          : 'Customer refund')}
                      {tx.agentName && <span className="text-[10px] text-slate-400 block">Agent: {tx.agentName}</span>}
                    </td>
                ...
                {(activeViewTab === 'all' || activeViewTab === 'purchases') && memberBills.map((b) => (
                  <tr key={b.id} className="bg-blue-50/40 hover:bg-blue-50 transition-colors">
                    <td className="p-2.5 font-mono text-[11px] text-blue-900 whitespace-nowrap">
                      {b.date}
                    </td>
                    <td className="p-2.5">
                      ...
                      <span className="block text-[10px] text-blue-700 font-mono mt-0.5 font-bold">
                        #{b.invoiceNo}
                      </span>
                    </td>
                    <td className="p-2.5 text-slate-800">
                      ...
                      <div className="text-[10px] text-slate-500 flex gap-2 mt-0.5">
                        ...
                        <span className="text-amber-700 font-semibold">Due: ₹{b.dueAmount.toLocaleString()}</span>
                      </div>
                    </td>
                    <td className="p-2.5 text-right font-extrabold text-blue-800">
                ...
                {memberTransactions.length === 0 && memberBills.length === 0 && (
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-slate-400">
                      ...
                      <p className="font-medium text-slate-600">No transactions or bills recorded yet.</p>
                      <p className="text-xs text-slate-400">Opening balance: ₹{member.openingAmt || member.netBalance || 0}</p>
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-200 text-xs">
          <div className="text-slate-500 font-medium">
            Member ID: <span className="font-mono text-slate-700 font-bold">{member.uniqueId || `CARD-${member.cardNumber}`}</span>
          </div>
          <button
            onClick={onClose}
            className="px-5 py-2 bg-slate-900 text-white rounded-xl text-xs font-bold hover:bg-black cursor-pointer transition-all shadow-xs"
          >
            Close Statement
          </button>
        </div>
