...
export const CustomerLedgerModal: React.FC<CustomerLedgerModalProps> = ({
  ...
  const ledgerItems: LedgerItem[] = useMemo(() => {
    ...
    const matchingBills = transactions.filter((t) => {
      ...
      if (t.customerPhone && custPhoneClean && t.customerPhone.replace(/\D/g, '') === custPhoneClean) return true;
      if (t.customerName && t.customerName.trim().toLowerCase() === custNameLower) return true;
      return false;
    });
    ...
    const matchingCardTx = cardTransactions.filter((ct: any) => {
      ...
      if (ct.customerPhone && custPhoneClean && ct.customerPhone.replace(/\D/g, '') === custPhoneClean) return true;
      const txNameLower = (ct.memberName || ct.customerName || '').trim().toLowerCase();
      if (txNameLower && txNameLower === custNameLower) return true;
      return false;
    });
    ...
    matchingBills.forEach((b) => {
      ...
      if (isReceiptOnly) {
        ...
        rawList.push({
          ...
          debit: 0,
          credit: b.payingNow || b.totalAmount || 0,
          mode: b.paymentMode,
        });
      } else {
  ...
  const totalBilled = useMemo(() => {
    ...
    return sumDebit > 0 ? sumDebit : effectivePurchased;
  }, [ledgerItems, customer]);

  const totalPaid = useMemo(() => {
    const sumCredit = ledgerItems.reduce((acc, i) => acc + i.credit, 0);
    return sumCredit > 0 ? sumCredit : (customer.totalPaid || 0);
  }, [ledgerItems, customer]);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl max-w-4xl w-full shadow-2xl overflow-hidden border border-slate-200 my-auto animate-fade-in flex flex-col max-h-[92vh]">
        ...
        <div id="printable-ledger" className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-5 bg-white text-slate-900">
          ...
          <div className="pt-4 border-t border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-4 items-end text-xs">
