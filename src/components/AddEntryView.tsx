    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="inline-flex items-center text-sm font-medium text-slate-600 hover:text-blue-600 transition self-start sm:self-auto cursor-pointer"
        >
          ← Back to Dashboard
        </button>
      </div>
      ...
      {savedEntry && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm animate-fade-in">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-700">
                Total: ₹{savedEntry.totalAmount.toLocaleString()} • Paid: ₹{savedEntry.payingNow.toLocaleString()} ({savedEntry.paymentMode})
                {savedEntry.dueAmount > 0 && ` • Due Balance: ₹${savedEntry.dueAmount.toLocaleString()}`}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-emerald-300 text-emerald-800 text-xs font-semibold hover:bg-emerald-100 transition cursor-pointer"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-xs font-semibold hover:bg-emerald-700 transition shadow-xs cursor-pointer"
            >
            ...
            <button
              ...
              onClick={resetForm}
              className="px-3 py-1.5 rounded-lg bg-slate-800 text-white text-xs font-medium hover:bg-slate-900 transition cursor-pointer"
            >
              + Next Entry
            </button>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sm:p-7">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-slate-50/70 border border-slate-200/80 rounded-xl p-4 space-y-2 relative">
            ...
            <div className="relative">
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2.5 text-sm bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 text-slate-800 placeholder-slate-400 transition"
                />
              </div>

              {/* Stock dropdown list */}
              {isStockDropdownOpen && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-30 max-h-56 overflow-y-auto divide-y divide-slate-100">
                  {filteredStock.length > 0 ? (
                    filteredStock.map((item) => (
                      <div
                        key={item.id}
                        onClick={() => handleSelectStock(item)}
                        className="p-2.5 hover:bg-blue-50/60 cursor-pointer flex items-center justify-between transition text-xs sm:text-sm"
                      >
                        <div>
            ...
            {selectedStockId && (
              <div className="mt-2 flex items-center gap-3 pt-2 border-t border-slate-200/60">
                <span className="text-xs font-medium text-slate-600">Quantity:</span>
                <div className="flex items-center gap-1.5">
                  <button
                    type="button"
                    onClick={() => handleStockQtyChange(stockQty - 1)}
                    className="w-7 h-7 rounded border border-slate-200 bg-white hover:bg-slate-100 font-bold text-slate-700 flex items-center justify-center cursor-pointer"
                  >
                    -
                  </button>
                  <input
                    ...
                    className="w-16 text-center py-1 text-xs border border-slate-200 rounded font-semibold text-slate-800"
                  />
                  <button
                    type="button"
                    onClick={() => handleStockQtyChange(stockQty + 1)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-20 max-h-48 overflow-y-auto divide-y divide-slate-100">
                  {filteredCustomers.map((c) => (
                    <div
                      ...
                      className="p-2.5 hover:bg-blue-50/60 cursor-pointer flex items-center justify-between text-xs transition"
                    >
                      <div>
                  ...
                  <div
                    ...
                    className="p-2 text-center text-xs text-blue-600 font-medium bg-slate-50 cursor-pointer hover:bg-blue-50"
                  >
            ...
            <div>
              ...
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  placeholder="0.00"
                  className="w-full pl-8 pr-4 py-2.5 text-sm bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 text-slate-900 font-medium placeholder-slate-400 transition"
                  required
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                ...
                <span className="text-slate-400 font-normal">(for invoice share)</span>
              </label>
              <input
                ...
                className="w-full px-3.5 py-2.5 text-sm bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 text-slate-800 placeholder-slate-400 transition"
              />
            </div>
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                {dueAmount > 0 ? (
                  <span className="text-[11px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200">
                    Remaining Udhar: ₹{dueAmount.toLocaleString()}
                  </span>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full p-3 text-sm bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 text-slate-800 placeholder-slate-400 resize-none transition"
              />
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 text-sm bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 text-slate-800 placeholder-slate-400 transition font-mono"
                />
              </div>
              ...
              <div>
                ...
                <div className="relative flex items-center">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2.5 text-sm bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 text-slate-800 transition"
                    required
                  />
                </div>
              </div>
          ...
          <div className="p-4 bg-slate-50/70 border border-slate-200 rounded-xl space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-slate-400">Card Schems 1, 2, 3</span>
            </div>
          ...
          {/* Payment Mode matching screenshot */}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 max-w-md">
              ...
              >
                <Banknote
                  className={`w-5 h-5 ${
                    paymentMode === 'Cash' ? 'text-emerald-600' : 'text-slate-400'
                  }`}
                />
                <span>Cash</span>
              </button>
              ...
              >
                <Smartphone
                  className={`w-5 h-5 ${
                    paymentMode === 'Online' ? 'text-blue-600' : 'text-slate-400'
                  }`}
                />
          ...
          <div className="pt-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <button
                ...
                id="btn-cancel-entry"
                onClick={onBackToDashboard}
                className="w-1/2 sm:w-auto px-5 py-2.5 rounded-xl border border-slate-200 text-slate-700 text-sm font-medium hover:bg-slate-50 transition cursor-pointer"
              >
                Cancel
              </button>
              <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-3">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
            <h2 className="text-sm font-bold text-slate-900">Today's Summary</h2>
