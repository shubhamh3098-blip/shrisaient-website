...
export const CustomersView: React.FC<CustomersViewProps> = ({
  ...
  const handleSendReminder = (c: Customer) => {
    ...
    const url = getSafeWhatsAppUrl(c.phone, text);
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl">
            ...
            <form onSubmit={submitAddCustomer} className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                >
                  {isSubmittingAdd ? 'सेव्ह होत आहे...' : 'Save Customer'}
                </button>
              </div>
            </form>
          </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className={`px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-semibold shadow-xs transition ${
                    isSubmittingSettle ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer active:scale-95'
                  }`}
                >
                  {isSubmittingSettle ? 'नोंद होत आहे...' : 'Record Payment'}
                </button>
              </div>
            </form>
          </div>
