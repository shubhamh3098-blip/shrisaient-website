...
import { exportSalesToCsv, exportReceiptsToCsv } from '../utils/csvExporter';
...
export const AllEntriesView: React.FC<AllEntriesViewProps> = ({
  ...
  const [entryToDelete, setEntryToDelete] = useState<TransactionEntry | null>(null);
  ...
  const exportCSV = () => {
    if (filterMode === 'Receipts') {
      exportReceiptsToCsv(filtered, 'ShriSai_Receipts');
    } else {
      exportSalesToCsv(filtered, filterMode === 'Bills' ? 'ShriSai_Sales_Bills' : 'ShriSai_All_Entries');
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800/90 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs overflow-hidden">
                    ...
                    <tr key={entry.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-750 transition">
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          {onEditEntry && (
                            <button
                              title="Edit bill / जुने बिल दुरुस्त करा"
                              onClick={() => onEditEntry(entry)}
                              className="p-1.5 rounded-lg text-slate-500 dark:text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 hover:bg-amber-50 dark:hover:bg-slate-700 transition cursor-pointer"
                            >
        ...
        {/* Delete Entry In-App Modal */}
        {entryToDelete && (
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-sm w-full p-6 shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-150">
              <div className="space-y-1">
                <h3 className="text-base font-bold text-slate-900 dark:text-white">
                  नोंद काढून टाकायची आहे का?
                </h3>
                <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                  बिल / पावती क्र. <strong>{entryToDelete.invoiceNo}</strong> ({entryToDelete.customerName}) ही नोंद सिस्टीममधून काढून टाकली जाईल.
                </p>
              </div>
        ...
        {filtered.length > PAGE_SIZE && (
          <div className="bg-slate-50 dark:bg-slate-850 border-t border-slate-200 dark:border-slate-750 px-4 py-3 flex flex-wrap items-center justify-between gap-3 text-xs">
            <div className="text-slate-600 dark:text-slate-400 font-medium">
              पान <strong className="text-slate-900 dark:text-white font-bold">{currentPage}</strong> पैकी{' '}
              <strong className="text-slate-900 dark:text-white font-bold">{totalPages}</strong> ({filtered.length} नोंदी)
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-1.5 rounded-xl text-xs font-bold border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition cursor-pointer"
              >
