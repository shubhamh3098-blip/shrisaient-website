...
export const AllEntriesView: React.FC<AllEntriesViewProps> = ({
  ...
  const [filterMode, setFilterMode] = useState<'All' | 'Cash' | 'Online' | 'Due'>('All');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          <button
            onClick={exportCSV}
            className="px-3.5 py-2 rounded-xl bg-white border border-slate-200 text-slate-700 text-xs font-semibold hover:bg-slate-50 transition flex items-center gap-1.5 cursor-pointer shadow-xs"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs">
          ...
          <p className="text-[11px] text-slate-400 mt-1">{filtered.length} entries shown</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs">
          ...
          <p className="text-[11px] text-emerald-700 mt-1">Cash & Online received</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs">
          <p className="text-xs text-slate-500 font-medium">Pending Udhar / Dues</p>
          <p className="text-xl font-bold text-amber-600 mt-0.5">
            ₹{totalPendingDues.toLocaleString()}
          </p>
          <p className="text-[11px] text-amber-700 mt-1">To be collected from clients</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs flex flex-col md:flex-row items-center gap-3">
        ...
        <div className="flex items-center gap-1.5 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
          {(['All', 'Cash', 'Online', 'Due'] as const).map((mode) => (
            <button
              key={mode}
        ...
        <div className="relative flex items-center w-full md:w-auto">
          ...
          <input
            ...
            className="pl-8 pr-2 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-lg text-slate-700 focus:outline-none focus:border-blue-600 w-full md:w-auto"
          />
          {filterDate && (
            <button
                ...
                filtered.map((entry) => (
                  <tr key={entry.id} className="hover:bg-slate-50/80 transition">
                    ...
                    <td className="py-3.5 px-4 max-w-xs">
                      <p className="text-slate-800 truncate">{entry.itemDetails}</p>
                      {entry.notes && (
